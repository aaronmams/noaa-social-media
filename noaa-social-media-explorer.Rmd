...
In the sample below, I have a table called "tweets_content" living in the PacFIN_Econ database on the Pinniger server.  
...
The reason I pulled down the existing data in the step above is that the Twitter accounts that I'm tracking aren't super active.  This means that if I pull down say the 200 most recent tweets two weeks in a row for the same user I'm likely to generate a lot of duplicated data.  To avoid saving a bunch of duplicates to my database table I use the 4-step process of 

1. get the existing data from the database
2. pull the new Twitter activity
